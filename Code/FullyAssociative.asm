...
tag_array: .word -1:8192                                 #note that we have set this for maximum possble index
#validity_array: .word 0:8192 
clock_array: .word 0:8192
...
initlp: 
	...
	#Now once we have the random byte address we want to calculate the tag
	#tag = byte addr/ block size
	...
	mflo   $s0             #quotient in $s0
    	...
    	#We want to scan the tag array for the tag, the tag is in $s0
    	li  $s3, 0
    	li  $s4, 0
    	la  $s2, tag_array             #load the address of the tag array
scantagarray:
    	beq  $s3, $s1, scantagarraycomplete
    	lw   $s5, 0($s2)
    	...
    	beq   $s5, $s0, hit
    	addi  $s3, $s3, 1      # loop counter incremented
    	addi  $s2, $s2, 4      #tag array counter increemented	
    	j scantagarray
scantagarraycomplete: 
	#we reached here and still could not find a hit so its a miss
	j miss

hit:    # $t1 stores 1 for LRU and zero for FIFO
	...
	j falldown
     LRU:
        ...
        la   $s7, clock_array             #load the address of the clock array
        #$s3 is the counter which when multiplied by four and added to $s7, will give us the position to store the clock($t0)
        mult $s3, $t4
        mflo $t9
...
miss: 
	...
	addi $k1, $k1, 1
	#scan the clock-array and find the least number, 
	li   $t9, 0                  #looping variable
	li   $t3, 1000001            #variable to do compute least count
	li   $s7, 0                  #clean the address variable
	li   $t5, 0                  #clean
	
	la   $s7, clock_array
	startscan:
	beq  $t9, $s1, donescan      #$s1 stores the array size
	lw   $a3, 0($s7)
	
	blt $a3, $t3, newleastcount
	j inc
	newleastcount:
	move $t3, $a3              #so my t3 contains the least element
	la $t5, ($s7)
	move $t6, $t9
	inc:
	addi $t9, $t9, 1
